...
public class HEFTPlanningAlgorithm extends BasePlanningAlgorithm {
    ...
    private Map<Host, List<Event>> schedules;
    ...
    public void run() {
        ...
        for (Object vmObject : Constants.hosts) {
            Host h = (Host) vmObject;
        ...
        for (Task task : getTaskList()) {
            ...
            for (Object vmObject : Constants.hosts) {
                ...
                if (vm.getNumberOfPes() < task.getNumberOfPes()) {
                    ...
                    costsVm.put(vm,
                            ((double)task.getCloudletTotalLength() / (double)(vm.getNumberOfPes() * vm.getPeList().get(0).getMips())));
                }
            }
     ...
     */
    private double findFinishTime(Task task, Host vm, double readyTime,
            boolean occupySlot) {
