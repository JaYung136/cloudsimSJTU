...
public final class WorkflowEngine extends SimEntity {
    ...
    protected void processJobReturn(SimEvent ev) {
        ...
        if (job.getCloudletStatus() == Cloudlet.FAILED) {
           ...
           // Log.printLine("jobs add : " + getJobsList().size());
           // Log.printLine("suc: " + sucTime);
            /*for(int i = 0; i < sucs.size(); i++) {
                Log.printLine(sucs.get(i) +"");
            }*/
            //jobsSubmitted ++;
        } else {
            sucTime ++;
            //sucs.add(job.getTaskList().get(0).getCloudletId());
        }
    ...
    protected void submitJobs() {
        Log.printLine("Workflow engine submit jobs");
