...
public class Host {
	...
	public double updateVmsProcessing(double currentTime) {
		...
		double cpuUtil = getCloudletScheduler().getCpuUtilization();
		double ramUtil = getCloudletScheduler().getRamUtilization();
		DecimalFormat df = new DecimalFormat("#.00");
		DecimalFormat dfs = new DecimalFormat("#.000");
		Log.formatLine(
				"%.2f: [Host #" + getId() + "] Total Utilization of MIPS for Containers is %.2f, Total Utilization of Ram for Containers is %.2f",
				CloudSim.clock(),
				cpuUtil,
				ramUtil);
		Constants.logs.add(new LogEntity(dfs.format(CloudSim.clock()), df.format(cpuUtil) , df.format (ramUtil)));
